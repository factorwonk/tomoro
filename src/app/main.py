...
def initialize_session_state():
    ...
    try:
        # First, load our environment variables
        load_environment()
        
        # Then proceed with RAG initialization
        if 'rag_chain' not in st.session_state:
            with st.spinner('Loading RAG model...'):
                try:
                    ...
                    st.info("Please check your OpenAI API key and environment setup.")
                    return False
        ...
        # This catches the error from load_environment if variables are missing
        st.error(str(e))
        return False
    except Exception as e:
